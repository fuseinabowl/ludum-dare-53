...
public class VertexPath : MonoBehaviour
{
    ...
    private Dictionary<Edge, GameObject> edgeModels = new Dictionary<Edge, GameObject>();
    ...
    {
        vertices.Clear();
        vertices.Add(edges[0].fromVertex);
    ...
    public bool CanSplit(Edge splitEdge)
    {
        ...
        if (edges[0].Equals(foundEdge) || LastEdge().Equals(foundEdge))
        {
            return false;
        }

        if (TrainIsOnEdge(foundEdge))
        {
            return false;
        }
    ...
    private Edge FindDirectionalEdge(Edge nonDirEdge, out int index)
    {
        Debug.Assert(nonDirEdge.direction == Edge.Direction.NONE);
        for (int i = 0; i < edges.Count; i++)
        {
            if (edges[i].NonDirectional().Equals(nonDirEdge))
            {
                index = i;
                return edges[i];
            }
    ...
    public bool DeleteEdge(Edge edge)
    {
        ...
        int i;
        DestroyEdge(FindDirectionalEdge(edge, out i));
        return true;
    }

    private void DestroyEdge(Edge edge)
    {
        GameObject.Destroy(edgeModels[edge]);
        edgeModels.Remove(edge);
        edges.Remove(edge);
        BuildVertices();
    }
